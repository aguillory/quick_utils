...
async function loadUserFarm() {
    try {
        ...
        const farmDoc = await window.getFarmCollection('farms')
            .doc(currentUser.uid)
            .get();
...
async function loadSpecies() {
    try {
        const speciesSnapshot = await window.getFarmCollection('species').get();
        allSpecies = [];
...
async function loadAnimals() {
    try {
        const animalsSnapshot = await window.getFarmCollection('animals')
...
async function loadAllFarms() {
    try {
        const farmsSnapshot = await window.getFarmCollection('farms').get();
...
deleteBtn.onclick = async () => {
    ...
    if (confirm('Are you sure you want to delete this animal? This action cannot be undone.')) {
        try {
            await window.getFarmCollection('animals').doc(editingAnimalId).delete();
            closeModal();
...
async function loadParentOptions(species) {
    try {
        const animalsSnapshot = await window.getFarmCollection('animals').where('species', '==', species.id).get();
