...
async function loadAllAnimals() {
    ...
    const snapshot = await window.getFarmCollection('animals').get();
...
async function loadSpecies() {
    const snapshot = await window.getFarmCollection('species').get();
...
async function saveHealthRecord(e) {
    ...
    try {
        if (recordId) {
            await window.getFarmCollection('healthRecords').doc(recordId).update(recordData);
...
async function saveHealthTask(e) {
    ...
    try {
        if (taskId) {
            await window.getFarmCollection('healthTasks').doc(taskId).update(taskData);
...
async function completeTask(e) {
    ...
    try {
        ...
        const recordRef = await window.getFarmCollection('healthRecords').add(recordData);
        
        await window.getFarmCollection('healthTasks').doc(taskId).update({
            status: 'completed',
    ...
    try {
        ...
        for (const animalId of selectedBulkAnimals) {
            ...
            if (recordType === 'record') {
                ...
                const recordRef = window.getFarmCollection('healthRecords').doc();
                batch.set(recordRef, {
                    animalId,
                    animalName: animal?.name || '',
                    farmId: currentFarmId,
...
async function deleteItem() {
    ...
    try {
        ...
        await window.getFarmCollection(collection).doc(itemId).delete();
